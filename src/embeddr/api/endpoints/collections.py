...
def create_collection(collection_data: CollectionCreate, session: Session = Depends(get_session)):
    collection = Collection(name=collection_data.name,
...
def list_collections(session: Session = Depends(get_session)):
    ...
    statement = select(Collection, func.count(CollectionItem.id)).outerjoin(
        CollectionItem).group_by(Collection.id)
...
def add_item_to_collection(
    ...
    existing = session.exec(select(CollectionItem).where(
...
def remove_item_from_collection(collection_id: int, image_id: int, session: Session = Depends(get_session)):
    item = session.exec(select(CollectionItem).where(
        CollectionItem.collection_id == collection_id,
        CollectionItem.image_id == image_id
    )).first()
    ...
    if not item:
        raise HTTPException(
            status_code=404, detail="Item not found in collection")
...
def get_collection_items(
    ...
    # Get images via join
    query = select(LocalImage).join(CollectionItem).where(
        CollectionItem.collection_id == collection_id)

    total = session.exec(select(func.count()).select_from(CollectionItem).where(
        CollectionItem.collection_id == collection_id)).one()
